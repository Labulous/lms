...
const getTodayDateRange = () => {
  ...
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
  return { todayStart: today.toISOString(), tomorrowStart: tomorrow.toISOString() };
}
...
const DailyCountsCard = ({
    ...
    const fetchDailyCounts = async () => {
      ...
      try {
        ...
        if (dueCases) {
          ...
          const pastDueCases = dueCases.filter((c) => {
            const dueDate = new Date(c.due_date);
            return dueDate < todayDateObj;
          });

          const dueTodayCases = dueCases.filter((c) => {
            const dueDate = new Date(c.due_date);
            return dueDate >= todayDateObj && dueDate < new Date(tomorrowStart);
          });
  ...
  console.log(metrics, "shipped cases dashboard =====================================>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
  const completedCases = metrics.casesCompleted;
  const totalDueCases = metrics.casesDue;
  const completionPercentage =
    totalDueCases > 0 ? Math.round((completedCases / totalDueCases) * 100) : 0;
  ...
  return (
    <Card className="bg-white h-full">
      ...
      <CardContent className="space-y-6">
        {/* Main Stats */}
        <div className="space-y-3 mt-2">
          {/* ... (rest of the main stats and progress bar remains the same) ... */}
          <div className="flex flex-col">
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-bold tracking-tight">
                {completedCases}/{totalDueCases}
        ...
        <div className="grid grid-cols-2 gap-1">
          ...
          {metricsDisplay.map((metric, index) => (
            // Special rendering for Cases Due to show breakdown
            metric.label === "Cases Due" ? (
              <div
                key={metric.label}
                className="flex items-start space-x-2 rounded-lg p-2 transition-colors hover:bg-gray-100 cursor-pointer"
                // Use the onClick defined in metricsDisplay or a default
                onClick={metric.onClick || (() => navigate("/cases"))}
              >
                <div className={`rounded-lg p-1.5 ${metric.bgColor}`}>
                  {metric.icon}
                </div>
                <div className="space-y-0.5">
                  ...
                  <div className="flex flex-col">
                    {/* Use the calculated total due from state */}
                    <p className="text-2xl font-bold">{metrics.casesDue}</p>
                    <div className="flex flex-col text-xs space-y-0.5 mt-0.5">
                      <div className="flex items-center space-x-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-red-500"></div>
                        {/* Use calculated past due from state */}
                        <span className="text-red-600 font-medium">
                          {metrics.pastDueCases} Past Due
                        </span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-gray-400"></div>
                        {/* Use calculated due today from state */}
                        <span className="text-gray-600 font-medium">
                          {metrics.dueTodayCases} Due Today
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              // Standard rendering for other cards
              <div
                key={metric.label}
                className="flex items-start space-x-2 rounded-lg p-2 transition-colors hover:bg-gray-100 cursor-pointer"
                // Use the onClick defined in metricsDisplay for each card
              ...
              >
                ...
                <div className="space-y-0.5">
                  ...
                  {/* Use the value from metricsDisplay which comes from state */}
                  <p className="text-2xl font-bold">{metric.value}</p>
                </div>
              </div>
            )
