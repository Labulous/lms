...
import { ShadeData } from './ShadeModal';
...
const SelectedProductsModal: React.FC<SelectedProductsModalProps> = ({
  ...
  const formatShadeDisplay = (shade?: ShadeData): string => {
    ...
    switch (type) {
      case '1':
        return shades.occlusal || 'No Shade';
      case '2':
        return `${shades.middle || 'No Shade'}`;
      case '3':
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${isExpanded ? 'h-[80vh]' : 'h-20'}`}>
      {/* Header/Pull Tab */}
      <div 
      ...
      {/* Products List */}
      {isExpanded && (
        <div className="p-6 overflow-auto" style={{ maxHeight: 'calc(80vh - 4rem)' }}>
          <div className="space-y-4">
            {products.map((product, index) => (
              <div key={`${product.productId}-${index}`} className="bg-gray-50 p-4 rounded-lg">
                <div className="flex justify-between items-start">
                  <div className="space-y-2">
                    <h3 className="font-medium text-gray-900">{product.productName}</h3>
                    <div className="space-y-1 text-sm text-gray-600">
                      <p>Billing Type: <span className="capitalize">{product.billingType}</span></p>
                      <p>Price: ${product.price.toFixed(2)}</p>
                      {product.discount > 0 && (
                        <p>Discount: {product.discount}%</p>
                      )}
                      <p>Final Price: ${calculateFinalPrice(product.price, product.discount).toFixed(2)}</p>
                      {product.teeth && product.teeth.length > 0 && (
                        <p>Selected Teeth: {product.teeth.join(', ')}</p>
                      )}
                      <p>Shade: {formatShadeDisplay(product.shade)}</p>
                    </div>
                  </div>
                  <button
