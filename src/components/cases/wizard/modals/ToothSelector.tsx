...
const ToothSelector: React.FC<ToothSelectorProps> = ({
  ...
  const [rangeSelections, setRangeSelections] = useState<Set<number>>(
    new Set()
  );
  const [individualSelections, setIndividualSelections] = useState<Set<number>>(
    new Set()
  );
  ...
  useEffect(() => {
    if (selectedProduct?.selectedPontic?.length > 0) {
      setPonticTeeth(new Set(selectedProduct?.selectedPontic));
    } else {
      setPonticTeeth(new Set());
    }
  }, [selectedProduct?.selectedPontic]);
  ...
  useEffect(() => {
    if (Array.from(ponticTeeth).length > 0) {
      setPonticMode(true);
    } else {
      setPonticMode(false);
    }
  ...
  const getTeethInVisualRange = (start: number, end: number) => {
    ...
    if (
      (isUpperArch(start) && isLowerArch(end)) ||
  ...
  const getToothColor = (toothNumber: number): string => {
    ...
    // if (addedTeethMap && addedTeethMap.has(toothNumber)) {
    //   const addedType = addedTeethMap.get(toothNumber)?.type?.[0];
  ...
  return (
    ...
    >
            ...
            {showTooltip && (
              <div className="absolute right-0 mt-1 p-2 bg-white border rounded-md shadow-lg z-50 w-48">
                <p className="text-xs mb-1">
                  Hold Shift and click two teeth to select a range.
                </p>
      ...
      >
        ...
        >
          {/* Selected Teeth Text */}
          <foreignObject x="65" y="60" width="136" height="210">
            <div
              ...
              return (
                <g key={`lower-${lowerToothNumber}`}>
                  <path
                    ...
                    onClick={(event) =>
                      handleToothClick(lowerToothNumber, event)
                    }
