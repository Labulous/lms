...
//                                 {row?.commonServices?.length} row Services
//                                 Added
//                               </span>
//                             ) : (
//                               "Add Services"
...
const ProductConfiguration: React.FC<ProductConfigurationProps> = ({
  ...
  useEffect(() => {
    console.log("fetchedServices==================================>>>>>", services, products)
  }, []);
  useEffect(() => {
    const getShadeOptions = async (labId: string) => {
      const shadeOptions = await fetchShadeOptions(labId);
                ...
                <React.Fragment key={row.id}>
                      ...
                      <Popover open={percentPopoverOpen.get(index) || false}>
                        ...
                        >
                          <div className="flex flex-col justify-between">
                            ...
                            <div>
                              <h2 className="text-sm font-bold">Product Prices</h2>
                              <Separator className="mt-2" />
                              <div className="mt-4 flex justify-between space-x-4">
                                <div>
                                ...
                                <div>
                                  ...
                                  <p className="text-sm font-extrabold text-blue-500">
                                    ${(row.price * (1 - (row.discount ?? 0) / 100)).toFixed(2)}
                                  </p>
                                </div>
                              </div>

                              <h2 className="text-sm font-bold mt-5">Service Prices</h2>
                              <Separator className="mt-2" />
                              <div className="mt-4 flex justify-between space-x-4">
                                <div>
                                  <Label className="text-xs text-gray-500">Price</Label>
                                  <p className="text-sm font-medium"> ${row.services?.reduce((sum, s) => sum + s.price, 0).toFixed(2)}</p>
                                </div>
                                <Separator orientation="vertical" className="h-8 mx-2" />
                                <div>
                                  <Label className="text-xs text-gray-500">Discount (%)</Label>
                                  <Input
                                    ...
                                    min="0"
                                    max="100"
                                    value={selectedProducts[index]?.subRows?.[0]?.services?.[0]?.discount ?? 0} // or services_discount if preferred
                                    onChange={(e) => {
                                      const updatedDiscount = Number(e.target.value);

                                      setselectedProducts((prevSelectedProducts: SavedProduct[]) => {
                                        return prevSelectedProducts.map((product, i) => {
                                          if (i !== index) return product;

                                          // Update main row services
                                          const updatedMainServices = product.services?.map(service => ({
                                            ...service,
                                            discount: updatedDiscount,
                                          })) ?? [];

                                          // Update all subRows' services and/or services_discount
                                          const updatedSubRows = product.subRows?.map(subRow => {
                                            const updatedServices = subRow.services?.map(service => ({
                                              ...service,
                                              discount: updatedDiscount,
                                            })) ?? [];
                                            ...
                                            return {
                                              ...subRow,
                                              services: updatedServices,
                                              services_discount: updatedDiscount, // optional if you use this field for binding
                                            };
                                          }) ?? [];
                                ...
                                <div>
                                  <Label className="text-xs text-gray-500">Total</Label>
                                  <p className="text-sm font-extrabold text-blue-500">
                                    ${((row.services?.reduce((sum, s) => sum + s.price, 0) ?? 0) * (1 - (selectedProducts[index]?.subRows?.[0]?.services_discount ?? 0) / 100)).toFixed(2)}
                                    {/* ${(
                                      row.price * (1 - (row?.services?.[0]?.discount ?? 0) / 100)
                                    ).toFixed(2)} */}
                                  </p>
                                </div>
                              </div>
                  ...
                  {expandedRows.includes(index.toString()) &&
                    ...
                    row?.subRows?.map((row_sub, subIndex) => {
                      let originalIndex = subIndex;
                      subIndex = subIndex + 100;
                      const subRow = selectedProducts[index]?.subRows?.[originalIndex];
                      const price = subRow?.price ?? 0;
                      const discount = subRow?.discount ?? 0;
                      const serviceDiscount = subRow?.services_discount ?? 0;  //subRow?.services?.[0]?.discount ?? 0;
                      return (
                        <TableRow key={subIndex} className="border pl-10">
                          <TableCell className="border-b"></TableCell>
                          <TableCell className="border-b">
                                ...
                                <div className="flex flex-col justify-between">
                                  <div className="flex justify-between">
                                    ...
                                    <Button
                                      size="sm"
                                      onClick={() => {
                                        handleDiscountChange(subIndex + index + 100 + originalIndex);
                                      }}
                                    >
                                      Save
                                    </Button>
                                  ...
                                  <div>
                                    <h2 className="text-sm font-bold">Product Prices sub</h2>
                                    <Separator className="mt-2" />
                                    <div className="mt-4 flex justify-between space-x-4">
                                      <div>
                                        <Label className="text-xs text-gray-500">Price</Label>
                                        <p className="text-sm font-medium">${row.price.toFixed(2)}</p>
                                      </div>
                                      <Separator orientation="vertical" className="h-8 mx-2" />
                                      <div>
                                        <Label className="text-xs text-gray-500">Discount (%)</Label>
                                        <Input
                                          ...
                                          max="100"
                                          value={
                                            selectedProducts[index]?.subRows?.[originalIndex]?.discount ?? 0
                                          }
                                            ...
                                            setselectedProducts((prevSelectedProducts: SavedProduct[]) => {
                                              return prevSelectedProducts.map((product, i) => {
                                                if (i === index) {
                                                  return {
                                                    ...product,
                                                    subRows: product.subRows?.map((subRow, sIdx) =>
                                                      sIdx === originalIndex
                                                        ? { ...subRow, discount: updatedDiscount }
                                                        : subRow
                                                    ),
                                                  };
                                                }
                                    ...
                                    <div className="mt-4 flex justify-between space-x-4">
                                      ...
                                      <div>
                                        ...
                                        <Input
                                          ...
                                          max="100"
                                          value={
                                            // selectedProducts[index]?.subRows?.[originalIndex]?.services?.[0]?.discount ?? 0
                                            selectedProducts[index]?.subRows?.[originalIndex]?.services_discount ?? 0
                                          }
                                          onChange={(e) => {
                                            const updatedDiscount = Number(e.target.value);

                                            setselectedProducts((prevSelectedProducts: SavedProduct[]) =>
                                              prevSelectedProducts.map((product, i) => {
                                                if (i === index) {
                                                  return {
                                                    ...product,
                                                    subRows: product.subRows?.map((subRow, sIdx) => {
                                                      if (sIdx === originalIndex) {
                                                        return {
                                                          ...subRow,
                                                          services_discount: updatedDiscount, // 👈 update this
                                                        };
                                                      }
                                                      return subRow;
                                                    }),
                                      ...
                                      <div>
                                        <Label className="text-xs text-gray-500">Total</Label>
                                        <p className="text-sm font-extrabold text-blue-500">
                                          {/* ${(price * (1 - serviceDiscount / 100)).toFixed(2)} */}
                                          ${((row.services?.reduce((sum, s) => sum + s.price, 0) ?? 0) * (1 - serviceDiscount / 100)).toFixed(2)}
                                        </p>
